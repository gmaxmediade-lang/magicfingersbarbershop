(() => {
  ...
  if (bookModal) {
    ...
    let demoMode = false; // becomes true if Cal.com isn't configured

    // No slot caching — always fetch live availability when entering step 3.

    // Shop opening hours — Magic Fingers Barbershop, Tue–Sat 10:00–18:00.
    const SHOP_HOURS = {
      2: [10, 18],
      3: [10, 18],
      4: [10, 18],
    ...
    const state = {
      ...
      timeIso: null,        // matching ISO from Cal.com slot response (or local-built in demo mode)
      barber: 'sheka',
      barberLabel: 'Sheka',
      name: '',
      email: '',
    ...
    function fetchEventTypes() {
      ...
      eventTypesPromise = fetch('/api/event-types')
        ...
        .then(data => {
          if (!data || !Array.isArray(data.eventTypes) || data.eventTypes.length === 0) {
            throw new Error('No Cal.com event types configured');
          }
        ...
        .catch(err => {
          // Cal.com not configured (no API key, or no event types yet).
          // Fall back to demo mode so the widget still showcases the flow.
          console.info('[booking] running in demo mode —', err.message);
          demoMode = true;
          eventTypes = Object.entries(SERVICE_SLUG).map(([service, slug], i) => ({
            id: 1000 + i,
            slug,
            title: slug,
            length: { 'cut': 30, 'cut-beard': 45, 'beard': 20, 'kids': 25 }[service] || 30
          }));
          return eventTypes;
        });
    ...
    async function buildTimes() {
      ...
      let slots;
      if (demoMode) {
        // Demo mode: synthesize available slots locally so the widget showcases
        // the full flow without needing a Cal.com account.
        ...
        if (range) {
          ...
          // Deterministic "taken" slots so the demo looks realistic
          const seed = state.date.getDate() + dow * 7;
          const taken = new Set();
          for (let i = 0; i < 3 + (seed % 3); i++) {
            const m = openH * 60 + ((seed * (i + 3) * 17) % ((closeH - openH) * 60));
            taken.add(Math.floor(m / inc) * inc);
          }
    ...
    async function submitBooking() {
      ...
      // Demo mode: skip Cal.com POST and go straight to confirmation.
      if (demoMode) {
